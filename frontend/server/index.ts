...
const isPage = (req: express.Request) => {
  let path = req.path;
...
const COHORT_MIDDLEWARES = COHORTS.map((cohort) =>
  createProxyMiddleware({
    target: cohort.url,
    changeOrigin: true,
    onProxyReq: (_proxyReq: express.Request, req: express.Request) => {
      req.id = uuid.v4();
      const callData = {
        path: req.path,
    ...
    onProxyRes: (proxyRes: express.Request, req: express.Request) => {
