...
export async function searchBlocks(keyword, height = -1, limit = 15) {
  try {
...
export async function getLatestBlocksInfo(limit = 15) {
  try {
    return await call(".select", [
      `SELECT blocks.*, COUNT(transactions.hash) as transactionsCount
        FROM (
...
export async function getPreviousBlocks(lastBlockHeight, limit = 15) {
  try {
    return await call(".select", [
      `SELECT blocks.*, COUNT(transactions.hash) as transactionsCount
        FROM (
